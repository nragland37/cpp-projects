...
// void printPaths(Node<Stock> *node, int path[], int pathLen, int lr, string &sentence);
void printPaths(Node<Stock> *node, int path[], int pathLen, string &sentence);
...
int main() {
    ...
    if (numStocks > 0) {
        ...
        // printPaths(root, path, 0, -1, sentence);
        printPaths(root, path, 0, sentence);
        cout << endl;
...
//             for (int i = 0; i < pathLen; ++i)
//                 sentence += to_string(path[i]);
//         } else {
//             printPaths(node->left, path, pathLen, 0, sentence);
//             printPaths(node->right, path, pathLen, 1, sentence);
//         }
//     }
// }
...
void printPaths(Node<Stock> *node, int path[], int pathLen, string &sentence) {
    if (node) {
        if ((!node->left) && (!node->right)) {
            ...
            path[pathLen] = 0;
            printPaths(node->left, path, pathLen + 1, sentence);
            path[pathLen] = 1;
            printPaths(node->right, path, pathLen + 1, sentence);
        }
    }
